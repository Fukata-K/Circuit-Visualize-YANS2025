...
# ripped from cmap since it doesn't play nice with new versions of matplotlib
def cmap(cmap_name, rgb_order=False):
    ...
    Args:
        ...
        rgb_order:
            boolean, if false or not set, the returned array will be in BGR order (standard OpenCV format).
            If true, the order will be RGB.
...
def color(cmap_name, index, rgb_order=False):
    ...
    Args:
        cmap_name: string, name of the colormap.
        index: floating point between 0 and 1 or integer between 0 and 255, index of the requested color.
        rgb_order:
            boolean, if false or not set, the returned list will be in BGR order (standard OpenCV format).
            If true, the order will be RGB.
