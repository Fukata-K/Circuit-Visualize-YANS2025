...
def get_scores_exact(
    ...
    Args:
        ...
        intervention (Literal['patching', 'zero', 'mean', 'mean-positional'], optional): the intervention to use. Defaults to 'patching'.
...
def get_scores_eap(
    ...
    for clean, corrupted, label in dataloader:
        ...
        clean_tokens, attention_mask, input_lengths, n_pos = tokenize_plus(
            model, clean, device=device
        )
        corrupted_tokens, _, _, _ = tokenize_plus(model, corrupted, device=device)

        (fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks), activation_difference = (
            make_hooks_and_matrices(
                model, graph, batch_size, n_pos, scores, device=device
            )
        )
...
def get_scores_eap_ig(
    ...
    Args:
        ...
        quiet (bool, optional): suppress tqdm output. Defaults to False.
        device (str, optional): device to use. Defaults to 'cuda'.
        prepend_bos (bool, optional): whether to prepend BOS token to inputs. Defaults to True.
    ...
    for clean, corrupted, label in dataloader:
        ...
        corrupted_tokens, _, _, n_pos_corrupted = tokenize_plus(
    ...
    for clean, corrupted, label in dataloader:
        ...
        with model.hooks(fwd_hooks=fwd_hooks_clean):
            ...
            activation_difference += (
                activations_corrupted.clone().detach()
                - activations_clean.clone().detach()
            )

        def output_interpolation_hook(
            k: int, clean: torch.Tensor, corrupted: torch.Tensor
        ):
        ...
        for node in nodeslist:
            for step in range(1, steps + 1):
                ...
                fwd_hooks = [
                    (
                        node.out_hook,
                        output_interpolation_hook(step, clean_acts, corrupted_acts),
                    )
                ]
...
def get_scores_clean_corrupted(
    ...
    """
    Gets scores using the clean-corrupted method: like EAP-IG,
    but just do it on the clean and corrupted inputs, instead of all the intermediate steps.
    ...
    for clean, corrupted, label in dataloader:
        ...
        with model.hooks(bwd_hooks=bwd_hooks):
            ...
            corrupted_metric_value = metric(
                corrupted_logits, clean_logits, input_lengths, label
            )
...
def get_scores_information_flow_routes(
    ...
    def make_hooks(
        n_pos: int, input_lengths: torch.Tensor
        ...
        def output_hook(index, activations, hook):
            try:
                ...
                print(
                    hook.name,
                    output_activations[:, :, index].size(),
                    output_activations.size(),
                )
                raise e
            ...
            try:
                ...
                proximity = torch.clamp(
                    -torch.linalg.vector_norm(
                        unsqueezed_output_activations[:, :, :prev_index] - acts,
                        ord=1,
                        dim=-1,
                    )
                ...
                # first mask out importance for padding positions
                ...
                importance *= mask
                importance = importance.sum(1) / input_lengths.view(
                    -1, 1, 1
                )  # mean over positions
                ...
                print(
                    hook.name,
                    unsqueezed_output_activations[:, :, prev_index].size(),
                    acts.size(),
                )
                raise e
        ...
        for layer in range(graph.cfg["n_layers"]):
            ...
            for i, letter in enumerate("qkv"):
                ...
                hooks.append(
                    (
                        node.qkv_inputs[i],
                        partial(input_hook, prev_index, bwd_index, input_lengths),
                    )
                )
        ...
        bwd_index = graph.backward_index(node)
        hooks.append(
            (node.in_hook, partial(input_hook, prev_index, bwd_index, input_lengths))
        )
        return hooks
...
def attribute(
    ...
    assert model.cfg.use_split_qkv_input, (
        "Model must be configured to use split qkv inputs (model.cfg.use_split_qkv_input)"
    )
    assert model.cfg.use_hook_mlp_in, (
        "Model must be configured to use hook MLP in (model.cfg.use_hook_mlp_in)"
    )
    ...
    if method == "EAP":
        ...
        scores = get_scores_clean_corrupted(
            model, graph, dataloader, metric, quiet=quiet, device=device
        )
        ...
        scores = get_scores_information_flow_routes(
            model, graph, dataloader, quiet=quiet
        )
    elif method == "exact":
