...
class Node:
    ...
    The in_hook is the TL hook into its inputs, while the out_hook gets its outputs.
    """
    ...
    @property
    def in_graph(self):
        return self.graph.nodes_in_graph[
            self.graph.forward_index(self, attn_slice=False)
        ]

    @in_graph.setter
    def in_graph(self, value):
        self.graph.nodes_in_graph[self.graph.forward_index(self, attn_slice=False)] = (
            value
        )
    ...
    def score(self, value):
        ...
        self.graph.nodes_scores[self.graph.forward_index(self, attn_slice=False)] = (
            value
        )
    ...
    def neurons(self, value):
        if self.graph.neurons is None:
            raise RuntimeError(
                f"Cannot set neurons for node {self.name} because the graph does not have neurons enabled"
            )
    ...
    def neurons_scores(self):
        if self.graph.neurons_scores is None:
            return None
        return self.graph.neurons_scores[
            self.graph.forward_index(self, attn_slice=False)
        ]
    ...
    def neurons_scores(self, value):
        if self.graph.neurons_scores is None:
            raise RuntimeError(
                f"Cannot set neurons_scores for node {self.name} because the graph does not have neurons_scores enabled"
            )
        self.graph.neurons_scores[self.graph.forward_index(self, attn_slice=False)] = (
            value
        )
...
class Graph:
    ...
    Attributes:
        ...
        n_backward (int):
            The number of backward nodes/indices in the graph, i.e. the # of nodes whose input gradients we care about.
            Note that attention heads have 3 inputs that need to be dealt with during a backward pass
    ...
    in_graph: (
        torch.Tensor
    )  # (n_forward, n_backward) tensor of whether the edge is in the graph
    ...
    nodes_in_graph: (
        torch.Tensor
    )  # (n_forward) tensor of whether the (source) node is in the graph
    ...
    def add_edge(
        self, parent: Node, child: Node, qkv: Optional[Literal["q", "k", "v"]] = None
    ):
    ...
    def _forward_index(cls, cfg, node_name: str, attn_slice: bool = False) -> int:
        """
        Given a model's config and a node specification, return the forward (source) index of the node in the graph.
        The forward index is the index of the node in the forward pass of the model, which is used to index into the graph's tensors.
    ...
    def _backward_index(cls, cfg, node_name: str, qkv=None, attn_slice=False) -> int:
        """
        Given a model's config and a node specification, return the backward (destination) index of the node in the graph.
        The backward index is the index of the node in the backward pass of the model, which is used to index into the graph's tensors.
    ...
    def apply_threshold(
        ...
        Args:
            ...
            reset (bool):
                resets the graph, setting everything to zero, before applying topn.
    ...
    def apply_topn(
        ...
        The components are specified by the level parameter, which can be 'edge','node', or 'neuron'.
        If 'node', the top-n nodes are selected based on their scores, and all outgoing edges to nodes in the graph are true.
        If 'edge', the top-n edges are selected based on their scores.
        If 'neuron', the top-n neurons are selected based on their scores, and all outgoing edges to nodes with neurons in the graph are true.
        ...
        Args:
            ...
            reset (bool):
                resets the graph, setting everything to zero, before applying topn.
                Only if reset=True will corresponding edges be added after neuron and node topn
            level (str, optional): level at which to apply topn. Defaults to 'edge'.
            prune (bool): whether to prune the graph after applying topn
        """
        if reset:
        ...
        if level == "neuron":
            ...
            assert n <= n_scored_neurons, (
                f"Requested n ({n}) is greater than the number of scored neurons ({n_scored_neurons})"
            )
            neuron_score_copy = self.neurons_scores.clone()
            if absolute:
                neuron_score_copy = torch.abs(neuron_score_copy)
            ...
            assert n <= n_scored_nodes, (
                f"Requested n ({n}) is greater than the number of scored nodes ({n_scored_nodes})"
            )
        ...
        # get top-n edges
            ...
            assert n <= self.real_edge_mask.sum(), (
                f"Requested n ({n}) is greater than the number of edges ({self.real_edge_mask.sum()})"
            )
    ...
    def prune(self):
        ...
        Then, it repeatedly removes nodes that lack incoming or outgoing edges (or both), and then edges missing a parent or child.
        ...
        while not old_new_same:
            ...
            nodes_with_ingoing[0] = (
                True  # input node always treated as if it has incoming edges
            )
            ...
            backward_in_graph = (
                self.nodes_in_graph.float() @ self.forward_to_backward.float()
            )
            ...
            old_new_same = torch.all(
                old_nodes_in_graph == self.nodes_in_graph
            ) and torch.all(old_edges_in_graph == self.in_graph)
    ...
    def from_model(
        ...
        graph.forward_to_backward = torch.zeros(
        ...
        if neuron_level:
            graph.neurons_in_graph = torch.zeros(
                (graph.n_forward, graph.cfg["d_model"])
            ).bool()
        ...
        for layer in range(graph.cfg["n_layers"]):
            attn_nodes = [
                AttentionNode(layer, head, graph)
    ...
    def to_pt(self, filename: str):
        ...
        """
        src_nodes = [
            node.name for node in self.nodes.values() if not isinstance(node, LogitNode)
        ]
    ...
    def from_json(cls, json_path: str) -> "Graph":
        ...
        The JSON should have the following keys:
            ...
            2. 'nodes':
                Dict[str, bool] which maps a node name (i.e. 'm11' or 'a0.h11') to a boolean value, indicating if the node is part of the circuit.
            3. 'edges': Dict[str, Dict] which maps an edge name ('node->node') to a dictionary contains values
            4. 'neurons':
                Optional[Dict[str, List[bool]]] which maps a node name (i.e. 'm11' or 'a0.h11') to a list of boolean values,
                indicating which of its neurons are part of the circuit.
        ...
        with open(json_path, "r") as f:
            d = json.load(f)
            assert all([k in d.keys() for k in ["cfg", "nodes", "edges"]]), (
                "Bad input JSON format - Missing keys"
            )
    ...
    def from_pt(cls, pt_path: str) -> "Graph":
        ...
        assert all([k in d.keys() for k in required_keys]), (
            f"Bad torch circuit file format. Found keys - {d.keys()}, missing keys - {set(required_keys) - set(d.keys())}"
        )
        assert d["edges_scores"].shape == d["edges_in_graph"].shape, (
            "Bad edges array shape"
        )
