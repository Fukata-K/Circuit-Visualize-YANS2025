...
from eap.graph import AttentionNode, Graph, LogitNode
...
def tokenize_plus(
    ...
    Args:
        ...
        device (Optional[Union[str, torch.device]]): The device to which the tokenized inputs will be moved. Defaults to 'cuda'.
...
def make_hooks_and_matrices(
    ...
    separate_activations = (
        model.cfg.use_normalization_before_and_after and scores is None
    )
    ...
    def gradient_hook(
        prev_index: int, bwd_index: Union[slice, int], gradients: torch.Tensor, hook
    ):
        """
    ...
    for layer in range(graph.cfg["n_layers"]):
        ...
        for i, letter in enumerate("qkv"):
            ...
            bwd_hooks.append(
                (node.qkv_inputs[i], partial(gradient_hook, prev_index, bwd_index))
            )
...
def compute_mean_activations(
    ...
    def activation_hook(index, activations, hook, means=None, input_lengths=None):
        ...
        if input_lengths is not None:
            ...
            item_means = einsum(
                acts,
                mask,
                "batch pos ... hidden, batch pos ... hidden -> batch ... hidden",
            )
    ...
    for batch in tqdm(dataloader, desc="Computing mean"):
        ...
        tokens, attention_mask, input_lengths, n_pos = tokenize_plus(
            model, batch_inputs, max_length=512, device=device, prepend_bos=prepend_bos
        )
        ...
        add_to_mean_hooks = [
            (
                hook_point,
                partial(
                    activation_hook, index, means=means, input_lengths=input_lengths
                ),
            )
