...
from eap.utils import compute_mean_activations, tokenize_plus
...
def make_hooks_and_matrices(
    ...
    Args:
        ...
        scores (Tensor):
            The scores tensor you intend to fill.
    ...
    def activation_hook(
        index, activations: torch.Tensor, hook: HookPoint, add: bool = True
    ):
    ...
    def gradient_hook(
        ...
        """
        Takes in a gradient and uses it and activation_difference to compute an update to the score matrix
...
def get_scores_eap(
    ...
    if neuron:
        scores = torch.zeros(
            (graph.n_forward, graph.cfg.d_model), device="cuda", dtype=model.cfg.dtype
        )
    else:
...
def attribute_node(
    ...
    if method == "EAP":
        ...
        scores = get_scores_eap_ig(
            model, graph, dataloader, metric, steps=ig_steps, quiet=quiet, neuron=neuron
        )
        ...
        raise ValueError(
            f"integrated_gradients must be in ['EAP', 'EAP-IG-inputs', 'EAP-IG-activations'], but got {method}"
        )
